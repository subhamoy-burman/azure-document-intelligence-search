...
def test_analyze_document():
    try:
        ...
        # OPTION 1: Use AnalyzeDocumentRequest with base64_source
        logging.info("TESTING OPTION 1: AnalyzeDocumentRequest with base64_source")
        try:
            # Convert bytes to base64
            base64_encoded = base64.b64encode(document_bytes).decode('utf-8')
            
            analyze_request = AnalyzeDocumentRequest(
                base64_source=base64_encoded
            )
        ...
        # OPTION 2: Direct method with file stream
        logging.info("TESTING OPTION 2: Direct method with file stream")
        try:
            with open(test_doc_path, "rb") as document_file:
                poller = document_intelligence_client.begin_analyze_document(
                    model_id="prebuilt-layout",
